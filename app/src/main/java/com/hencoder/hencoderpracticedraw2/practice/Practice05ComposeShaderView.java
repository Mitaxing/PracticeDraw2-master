...
public class Practice05ComposeShaderView extends View {
    ...
    private void init() {
        Bitmap bm = BitmapFactory.decodeResource(getResources(), R.drawable.batman);
        Shader shader = new BitmapShader(bm, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);

        bm = BitmapFactory.decodeResource(getResources(), R.drawable.batman_logo);
        Shader shader1 = new BitmapShader(bm, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);

        composeShader = new ComposeShader(shader, shader1, PorterDuff.Mode.DST_ATOP);
    }
